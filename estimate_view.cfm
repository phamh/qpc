	...
	<cfif #Evaluate(Int(trips))# NEQ "0">
		...
		<cfelseif Len(Int(trips)) EQ "2" AND mid(Int(trips), 1, 1) NEQ "0">
			...
			<cfelse>
				...
				<cfif mid(trips, 2, 1) NEQ "0">
					<span style="display: #variables.display#">#listGetAt(write_single, mid(trips, 2, 1), ',')#</span>
                    <cfset variables.totalText = variables.totalText & ' ' & listGetAt(write_single, mid(trips, 2, 1), ',')>
				</cfif>
			</cfif>
...
<script>
	...
	duplicateEstimate = function(estimateId)
	{
		try
		{
			theConfirm = confirm('Do you want to duplicate the estimate \"'+ estimateId + '\"?');
			if(theConfirm == true)
			{
				var e = new qpcCFC();
				e.setHTTPMethod("POST");
				theToken = document.getElementById('token').value;
				jsDuplicate = e.function_duplicate(estimateId, theToken);	
				gFormChanged = false;
		...
		{
			alert('error on function duplicateEstimate: '+ error.message);
		}
		return true;
	}
	...
	<cfif url.printPDF EQ 0>
        ...
        <table border=0 style="width:990px;">
        	<td style="width:350px">
					...
					<input type="button" value="Edit" class="buttonSmall2" onclick="editThisEstimate('#url.id#')">
						
					<input type="button" value="Duplicate This Estimate" class="buttonSmall2" onclick="duplicateEstimate('#url.id#')">
							
					<input type="button" value="Preview In PDF" class="buttonSmall2" onclick="printThisEstimate('#url.id#')">
            ...
            <td style="width:350px;text-align:right">
            	<input type="button" value="Delete" class="buttonSmall_delete" onclick="deleteThisEstimate('#getThisEstimate.estimateID_pk#')">&nbsp;
		...
		<table style="width:990px; border-collapse:collapse;border:0px solid black; margin-bottom:70px">
			...
			<tr>
				<td style="border:0px solid black; font-size: #variables.fontSize#">503 Arch Ridge Loop</td>
			</tr>	
			<tr>
				<td style="border:0px solid black; font-size: #variables.fontSize#">Seffner, FL. 33584</td>
		...
		<table style="width:990px; border-collapse:collapse;border:0px solid black; margin-bottom:20px">
			...
			<tr>
				...
				<cfif url.printPDF EQ 0>
					<td rowspan="4">
						<b>Estimate Status:</b> 
						<cfswitch expression="#getThisEstimate.estimateStatus#">
							<cfcase value="0">
								Pending
							</cfcase>
							<cfcase value="1">
