...
*/
	...
	submitNewInvoice=function(thisWindowName)
	{
		var e = new qpcCFC();
		...
		var serviceDateListArray = new Array();
		var productServiceListArray = new Array();
		var descriptionListArray = new Array();
		var amountListArray = new Array();
		...
		{       
			...
			{						
				serviceDateListArray[arrayIndex] = document.getElementById("serviceDate_"+i).value;
				productServiceListArray[arrayIndex] = document.getElementById("product_"+i).value;	
				descriptionListArray[arrayIndex] = document.getElementById("description_"+i).value;			
				amountListArray[arrayIndex] = document.getElementById("amount_"+i).value;																
			}
		arrayIndex++;	
		}	

		var addNewInvoice = e.function_addNewInvoice
							(
								invoiceNumber,
								customerId,
								term,
								...
								project,
								location,
								serviceDateListArray,
								productServiceListArray,
								descriptionListArray,
								amountListArray,
								theToken
							);
						
		if(addNewInvoice != 'no error')
			alert(addNewInvoice)		
		else	
		{
			alert('New Invoice is submitted.')
			closeThisCfWindow(thisWindowName, 'invoiceContent.cfm', 'invoiceContentBody');	
		}	
	}

	function calculateInvoiceTotal(value)
	{
		try
		{
			var arrayIndex = 0;
...
<cfif getLastInvoiceNumber.invoiceNumber EQ ''>
	<cfset variables.invoiceNumber = 2000>
<cfelse>
	<cfset variables.invoiceNumber = getLastInvoiceNumber.invoiceNumber + 1> 
</cfif>

<div style="padding-left:45px; margin-bottom:5px;  border:0px solid gray;width:99%; text-align:left; padding-top:5px">
	<input type="button" value="Submit"  class="buttonSmall2" onclick=submitNewInvoice('new_invoice_cfWidnow');> 
	<input type="button" value="Cancel"  class="buttonSmall2" onclick=closeThisCfWindow('new_invoice_cfWidnow','','')> 
</div>
...
<table class="invoiceTable" align="center" border="1" style="margin-bottom:20px">
	...
	<tr style="vertical-align:top">
		<td style="width:140px; text-align:center; font-weight:bold" id="invoiceNumber">
			<cfoutput>#variables.invoiceNumber#</cfoutput>
		</td>
		<td style="width:200px">
			<select id="company" style="width:200px" onchange="selectThisCustomer(this.value)">
				<option value="0">Select a customer</option>
				<cfoutput query="GetCustomerList">
					<option value="#customerId_pk#">#company#</option>
				</cfoutput>
				
			</select>
			<input id="companyAddress" style="width:200px; border:0px solid gray;background-color:white" disabled="disabled"></input><br>
			<input id="companyState" style="width:200px; border:0px solid gray;background-color:white" disabled="disabled"></input><br>
			<input id="companyZip" style="width:200px; border:0px solid gray;background-color:white" disabled="disabled"></input>
		</td>
		...
		<td>
			<input id="invoiceDate" style="width:150px" type="date" value="<cfoutput>#dateFormat(now(), 'yyyy-mm-dd')#</cfoutput>" onchange="updateDueDate()"></input>
		</td>

		<td>
			<input id="dueDate" style="width:100px; border:0px solid gray" type="text" value="<cfoutput>#dateFormat(now(), 'mm/dd/yyyy')#</cfoutput>" readonly="readonly"></input>
		</td>
				
		<td><input id="project" style="width:170px" maxlength="100"></input></td>
		<td><input id="location" style="width:170px" maxlength="100"></input></td>
	</tr>
</table>
...
<table align="center" style="border:0px slid gray; width:<cfoutput>#tableWidth#</cfoutput>; margin-top:10px">
	...
	<tr>
		...
		<th style="width:15%; border:0px solid black; text-align:right" id="invoiceTotal"><span id="invoiceTotal">0.00</span></th>
	</tr>		
	</cfoutput>
</table>