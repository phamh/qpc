	...
	updateThisEstimate=function(estimateId)
	{
		try
		{
			...
			theToken = document.getElementById('token').value;
			 ...
			 if(document.getElementById('estimateStatus2').checked == true)
				var estimateStatus = 1;
			else if(document.getElementById('estimateStatus3').checked == true)
			...
			else
			{
				...
				js_updateEstimate = e.function_updateEstimate(editEstimateId,estimateNumber,estimateDate, companyName, attention, 
															companyAddress,jobsiteAddress,expiredDays,preparedBy,
															 ...
															 editRemarkArray,
															 deleteTheseRemark,
															 newItemArray,newScopeWorkArray,newAmountArray,newRemarkArray, estimateStatus,theToken);
															 
				if(js_updateEstimate == 'hacker')
				{
					var e = new qpcCFC();
					e.setHTTPMethod("POST");
					...
					ColdFusion.navigate('hacker.cfm');
				}
				else
				{
...
<cfoutput>
    ...
    token= <input type="text" name="token" id="token" value="#CSRFGenerateToken()#" size="50">
</cfoutput>
</div>
...
<div align=center style="border:2px solid black; width:1000px; height:auto;padding:5px">
		...
		<table border=0 style="width:#tableWidth#">
        	<td style="width:300px">
				<input type="button" value="Update" class="buttonSmall2" onclick="updateThisEstimate('#url.id#')">
				<input type="button" value="Worksheet" class="buttonSmall2" onclick="PopupCenter('estimate_worksheet.cfm?estimateId=#url.id#', 'worksheetWindow', '1000','450')('#url.id#')">
				<input type="button" value="Cancel" class="buttonSmall2" onclick="cancelNewEstimate('#url.id#')">          
            </td>
	...
	<table style="width:<cfoutput>#tableWidth#</cfoutput>; border-collapse:collapse;border:0px solid black; margin-bottom:30px">
		<cfoutput>
		<tr>
		...
		<tr>
			...
			<td style="border:#borderWidth# solid black">
				<input  id="estimateDate"  name="estimateDate" value="#dateFormat(getThisEstimate.estimateDate, 'YYYY-MM-DD')#" type="date"  style="width:160px"></input>
<!---				<input type="text" id="estimateDate"  name="estimateDate" value="#dateFormat(getThisEstimate.estimateDate, 'mmm dd, yyyy')#" readOnly size="10"
	...
	<table style="width:<cfoutput>#tableWidth#</cfoutput>; border-collapse:collapse;border:0px solid black; margin-bottom:20px">
		...
		<tr>
			...
			<td rowspan="4" style="border:0px solid black; text-align:left">
				<fieldset>
					<legend style="color:red">Estimate Status</legend>
					<input   type="radio" id="estimateStatus1"  name="estimateStatus" value="0"  <cfif getThisEstimate.estimateStatus EQ 0>checked</cfif> /> Pending
					<input  type="radio" id="estimateStatus2"  name="estimateStatus" value="1"  <cfif getThisEstimate.estimateStatus EQ 1>checked</cfif>/> Approved
					<input  type="radio" id="estimateStatus3"  name="estimateStatus" value="2"  <cfif getThisEstimate.estimateStatus EQ 2>checked</cfif>/> Rejected
				</fieldset>
			</td>
		</tr>
	...
	<table style="width:<cfoutput>#tableWidth#</cfoutput>; border-collapse:collapse;border:1px solid black; margin-bottom:5px" id="estimateTable">
		...
		<cfoutput query="getThisEstimateItems">
			...
			<tr id="trEditItem_#currentRow#" valign="top">
				...
				<td style="border:1px solid black;" class="td_input">
					<input id="editAmount_#currentRow#" type="text" value="#amount#" size="10" style="text-align: right" 
